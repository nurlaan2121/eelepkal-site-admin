...
export const createVenueCrudApi = () => {
  return {
    ...
    addVenueHours: async (
      ...
      // Превращаем красивый объект обратно в "плоский" формат бэкенда
      const flatHours = Object.entries(hours).reduce((acc, [day, time]) => {
        acc[`${day}Open`] = time.open;
        acc[`${day}Close`] = time.close;
        return acc;
      }, {} as any);
      console.log(flatHours);
