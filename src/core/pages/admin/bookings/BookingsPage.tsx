...
import {
  adminBookingService,
  BookingResponse,
} from "@/api/booking/adminBookingService";
...
export const AdminBookingsPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "WAITING" | "APPROVED" | "REJECTED" | "COMPLETED" | "NOT_PAID" | "ALL"
  >("ALL");
  ...
  const {data: bookingDetails, isLoading: isDetailsLoading} = useQuery({
    ...
    queryFn: () =>
      adminBookingService.getBookingDetails(selectedBooking!.bookingId),
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 font-bold tracking-wide uppercase mt-0.5">
                        {booking.typeClientResponse === "NEW"
                          ? "🆕 Новый клиент"
                          : "⭐ Постоянный клиент"}{" "}
                        • {booking.clientAge} лет
                      </p>
                    </div>
                    <span
                      className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${statusInfo.color}`}
                    >
                      {statusInfo.label}
                    </span>
                  </div>
                ...
                bookings.map((booking) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter border ${statusInfo.color}`}
                        >
                          {statusInfo.label}
                        </span>
                      </td>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      #
                      {bookingDetails?.bookingCode || selectedBooking.bookingId}
                    </p>
                  </div>
                  <button
                    onClick={() => setIsDetailModalOpen(false)}
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {bookingDetails.typeClientResponse === "NEW"
                        ? "🆕 Новый клиент"
                        : "⭐ Постоянный клиент"}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                      ...
                      <p className="text-sm font-black text-gray-900">
                        {formatDate(bookingDetails.bookingFullVisitTime)}
                      </p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                      ...
                      <p className="text-sm font-black text-gray-900">
                        {bookingDetails.countOfGuests} чел.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <span
                      className={`inline-flex px-3 py-1.5 rounded-full text-xs font-black uppercase tracking-wider ${getStatusInfo(bookingDetails.bookingStatus).color}`}
                    >
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <p className="text-sm font-bold text-gray-900">
                      {formatDate(bookingDetails.bookingCreatedAd)}
                    </p>
                  </div>
                </div>
              ) : null}
