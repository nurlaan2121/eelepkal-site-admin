...
import {MenuItem, MenuStatus, adminMenuService} from "@/api/admin/menu";
...
const EmptyState: React.FC<{status: MenuStatus}> = ({status}) => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    ...
    <h3 className="text-lg font-black text-slate-900 mb-2">
      {status === "ACTIVE"
        ? "В этой категории пока нет активных блюд"
        : "В этой категории пока нет резервных блюд"}
    </h3>
    ...
    >
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
          <div>
            <h3 className="font-black text-slate-900 text-base line-clamp-1 mb-1">
              {item.title}
            </h3>
          ...
          <p className="text-xl font-black text-brand-primary">
            {item.price}{" "}
            <span className="text-xs font-bold text-slate-400">с</span>
          </p>
        </div>
...
export const AdminMenuPage: React.FC = () => {
  ...
  const {data: categories, isLoading: categoriesLoading} = useQuery({
    ...
    queryFn: devService.getCategories,
  });
  ...
  const handleMove = (menuId: number) => {
    ...
    const currentLabel =
      currentStatus === "ACTIVE" ? "активного" : "резервного";
      ...
      {categoriesLoading ? (
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {Array.from({length: 5}).map((_, i) => (
            <div
              key={i}
              className="px-5 py-3 rounded-2xl bg-slate-200 animate-pulse min-w-[120px]"
            />
          ))}
        </div>
      ) : (
            ...
            <AnimatePresence>
              {menuItems.map((item) => (
                <MenuCard
                  ...
                  isMoving={
                    moveMutation.variables?.menuId === item.id &&
