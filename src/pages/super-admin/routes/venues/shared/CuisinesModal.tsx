import {superAdminVenueService, VenueListItem} from "@/api/super-admin/venue";
...
export const CuisinesModal = ({
  ...
  const {data: allCuisines, isLoading: isLoadingAll} = useQuery({
    ...
    queryFn: () => devService.getAllCuisines(),
  });

  const {isLoading: isFetchingCurrent, data: venueCuisines} = useQuery({
    queryKey: ["venue-cuisines", venueId],
    queryFn: async () => {
      const data = await superAdminVenueService.getCuisines(venueId);
      setSelectedIds(data || []);
      return data;
    },
  });
  ...
  useEffect(() => {
    if (allCuisines && venueCuisines) {
      setSelectedIds(
        allCuisines
          .filter((el) => venueCuisines.includes(el.id))
          .map((el) => el.id),
      );
    }
  }, [allCuisines, venueCuisines]);
  const mutation = useMutation({
    mutationFn: () =>
      superAdminVenueService.addVenueCuisines(venueId, {
