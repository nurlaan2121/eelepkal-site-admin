        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                {displayedAmenities.map((aId) => {
                    ...
                    return (
                        <div key={aId} className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/50 border border-slate-100/50 transition-all hover:bg-white hover:shadow-md group">
                            <VenueAmenityChip id={aId} name={found?.name || `Услуга ${aId}`} minimized />
                        </div>
                    );
                })}
            </div>
