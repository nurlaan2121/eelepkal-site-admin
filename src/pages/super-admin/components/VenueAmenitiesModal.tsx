                    ...
                    >
                        ...
                        <div className="flex items-center justify-between p-5 border-b border-slate-100">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-black text-slate-900">Удобства и услуги</h2>
                                    <p className="text-xs text-slate-400 font-medium">Выберите всё, что есть в заведении</p>
                                </div>
                            </div>
                                ...
                                {filteredAmenities.map((amenity) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`group flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${isSelected
                                                    ? 'bg-brand-primary/5 border-brand-primary text-brand-primary shadow-lg shadow-brand-primary/5'
                                                    : 'bg-white border-slate-100 text-slate-600 hover:border-slate-200 hover:bg-slate-50'
                                                }`}
                                        >
                                            <div className="flex items-center gap-3 text-left">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm ${isSelected ? 'bg-brand-primary text-black font-black' : 'bg-slate-100 text-slate-400 font-bold'}`}>
                                                    {amenity.name.charAt(0)}
                                                </div>
                                                <span className="text-sm font-black tracking-tight">{amenity.name}</span>
                                            </div>
                                            <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-brand-primary border-brand-primary' : 'border-slate-200 bg-white group-hover:border-slate-300'
                                                }`}>
                        ...
                        <div className="p-5 border-t border-slate-100 bg-white">
                            ...
                            >
                                {isSaving ? 'Сохранение...' : `Сохранить изменения (${selectedIds.length})`}
                            </Button>
                        </div>
                    </motion.div>
                </div>
