...
// ─────────── Replace Admin Modal ───────────
const ReplaceAdminModal: React.FC<{
  venue: VenueListItem;
  onClose: () => void;
}> = ({venue, onClose}) => {
                  ...
                  <button
                    onClick={() =>
                      setForm((f) => ({
                        ...
                        cancellationDeadline: f.cancelAllowed
                          ? "00:00"
                          : "03:00",
                      }))
                    }
...
// ─────────── Cuisines Modal ───────────
const CuisinesModal: React.FC<{venue: VenueListItem; onClose: () => void}> = ({
  venue,
  onClose,
}) => {
...
const PaymentModal: React.FC<{venue: VenueListItem; onClose: () => void}> = ({
  venue,
  onClose,
}) => {
  const queryClient = useQueryClient();
  ...
  const addMutation = useMutation({
    mutationFn: () =>
      superAdminVenueService.addPaymentDetail(venue.venueId, formData),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (paymentId: number) =>
      superAdminVenueService.updatePaymentDetail(paymentId, formData),
    onSuccess: () => {
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {isLoading ? (
              ...
              payments.map((p) => (
                <div
                  key={p.id}
                  className="relative bg-gradient-to-br from-brand-950 to-brand-700 rounded-2xl p-5 text-white space-y-4"
                >
  ...
  const actions = [
    {
      ...
      modal: "replace-admin" as ModalType,
      color: "text-brand-600",
    },
    {
      icon: Utensils,
      label: "Тип кухни",
      modal: "cuisines" as ModalType,
      color: "text-orange-600",
    },
    {
      icon: Settings2,
      label: "Условия бронирования",
      modal: "conditions" as ModalType,
    ...
    <>
      ...
      <AnimatePresence>
        ...
        {activeModal === "conditions" && (
          <ConditionsModal venue={venue} onClose={() => setActiveModal(null)} />
        )}
        {activeModal === "payment" && (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">
            {venues.length} заведений в системе
          </p>
        </div>
        <Button
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        {isLoading ? (
          Array.from({length: 4}).map((_, i) => (
            <div
              key={i}
              className="p-5 border-b border-slate-100 animate-pulse"
            >
